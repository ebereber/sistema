...
export type LocationInsert = Omit<Location, "id" | "created_at" | "updated_at" | "points_of_sale">
 ...
 * If is_main is true, unmarks other locations as main
 */
export async function updateLocation(id: string, location: LocationUpdate): Promise<Location> {
  const supabase = createClient()
...
export async function archiveLocation(id: string): Promise<void> {
  ...
  const { error } = await supabase
    .from("locations")
    .update({ active: false, is_main: false, updated_at: new Date().toISOString() })
    .eq("id", id)
